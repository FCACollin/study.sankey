...
ads %>%
  head() %>%
  knitr::kable(caption = "Outlook of the data")
...
- needs functions define in other projects:
    + _"Oh! I remember, I already coded this functionalities for a previous 
    analysis!"_:
...
2. Move the script in `inst/`, a rather permissive area of the package.
    + makes sections ot enable show/hide document outline in RStudio interface,
    and make section _knit-able_.
    e.g.
...
- "_Many files, will not this be complicated to find a file in the future?"_
...
- "_Many files, how a programmer/reviewer knows where to start?_"
    ...
    screen the script sections, and use empty named r code chunk to
    decide which section to execute in which order.
    + Good because:
        - there are sections needed for development only, but not for the
        program run (e.g. preview) while code-commenting is bad practice.
...
- `devtools`:
    ...
    updates the function documentation
    (populates the `NAMESPACE` file and `man/` folder).
    + `<ctrl>+<shift>+<e>`, equivalent to `devtools::check()` very a large
    number of standards (e.g. functions are well documented, dependencies are
    well accounted for, examples are working).

![Devtools: look at the build panel](img/rig_06.png){ width=50% }

- `lintr`:
    + helps checking the code is lint-free, clean. Helps in the adoption of
    good practices and standard.

![Lintr: keep-up with code quality](img/rig_07.png){ width=50% }


- `pkgdown`:
    + build an website: the result `README.md` becomes the home page, functions
